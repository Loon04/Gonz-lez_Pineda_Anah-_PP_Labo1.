...
def funcion_leer_json_array(ubicacion_archivo:str):
    """abro el archivo y lo paso al codigo 

    Args:
        ubicacion_archivo (str): ubicacion del archivo (Ruta absoluta o relativa)

    Returns:
        _type_: FALSE si hubo algun error y TRUE se retorna la lista
    """
    retorno = False
    try:
...
def normalizar_datos(lista:list): #revisar
    ...
    if lista == []:
        ...
        for datos in lista:
            if type(datos["cantidad"]) != int:
                datos["cantidad"] = int(datos["cantidad"])
                normalizar_datos = True
                #print(type(personajes["peso"]))
            if type(datos["precioUnitario"]) != float:
                datos["precioUnitario"] = float(datos["precioUnitario"])
                normalizar_datos = True
            else:
                normalizar_datos = False
